...
\begin{enumerate}[label=(\arabic*)]
	...
	\item 光的波粒二象性：粒子性：黑体辐射、光电效应；波动性：干涉衍射、光学显微镜、光谱\\
				...
				则有\[I = A_1^2+A_2^2+2A_1A_2\cos \ubelownote{(k\var L)}{\text{光程差}}\]
...
\begin{enumerate}[label=(\arabic*)]
	...
	\item 讨论波函数随时空的变化：\\
				时间：\inlineequation[time]{E\Psi = \i\hbar\pdv{\Psi}{t}}\\
				空间：\inlineequation[space]{\vec{p}\Psi = -\i\hbar\nabla\Psi}\\
				对空间做两次微分：\inlineequation[two-time-pdv]{(\i\hbar\nabla)^2 \Psi = \vec{p}^2 \Psi \Rightarrow \nabla^{2}=\frac{-p^2}{\hbar^2}\Psi \Rightarrow \vec{p}^2\Psi=-\hbar^2\nabla^2\Psi}\\
				考虑一个非相对论情况：\inlineequation[non-relativity]{E=\frac{\vec{p}^2}{2m}}
	\item 薛定谔方程：\begin{enumerate}[label=(\alph*)]
				...
				一般情况下，粒子处在一个势场中：\\
				\inlineequation[modified-non-relativity]{E = \ubelownote{\frac{\vec{p}^2}{2m}}{动能}+\dabovenote{U}{势能}}（改写\eqref{non-relativity}）\\
